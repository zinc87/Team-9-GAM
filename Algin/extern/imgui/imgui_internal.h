...
// AG: we can't rely on ImVec2 math operators being available here!
...
{
    // AG: need to be in sync with last value of ImGuiSelectableFlags_
...
enum ImGuiDockNodeFlagsPrivate_
{
    ...
    ImGuiDockNodeFlags_HiddenTabBar             = 1 << 13,  // Saved // Tab bar is hidden, with a triangle in the corner to show it again (AG: actual tab-bar instance may be destroyed as this is only used for single-window tab bar)
...
{
    ImGuiID     ID;
    ImVec2ih    Pos;            // AG: Settings position are stored RELATIVE to the viewport! Whereas runtime ones are absolute positions.
    ImVec2ih    Size;
...
struct ImGuiContext
{
    ...
    bool                    NavMousePosDirty;                   // When set we will update mouse position if (io.ConfigFlags & ImGuiConfigFlags_NavEnableSetMousePos) if set (AG: this not enabled by default)
    bool                    NavDisableHighlight;                // When user starts using mouse, we hide gamepad/keyboard highlight (AG: but they are still available, which is why NavDisableHighlight isn't always != NavDisableMouseHover)
...
//#endif
    ...
    // AG: All position are in absolute pixels coordinates (we are never using window coordinates internally)
