...
// AG: This is _not_ a full expression evaluator. We should probably add one and replace this dumb mess..
...
// AG: You likely want to specify the ImGuiSliderFlags_AlwaysClamp when using this.
...
// AG: v_speed is float to allow adjusting the drag speed with more precision
...
static bool InputTextFilterCharacter(ImGuiContext* ctx, unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, bool input_source_is_clipboard)
{
    ...
    // Filter non-printable (AG: isprint is unreliable! see #2467)
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    // AG: The flag test is merely an optional micro-optimization, BeginDragDropTarget() does the same test.
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    // AG: The ActiveId test is merely an optional micro-optimization, BeginDragDropSource() does the same test.
...
bool ImGui::TreeNodeUpdateNextOpen(ImGuiID storage_id, ImGuiTreeNodeFlags flags)
{
    ...
    // AG- If we are above max depth we still allow manually opened nodes to be logged.
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, const ImVec2& size_arg)
{
    ...
    if (values_count >= values_count_min)
    {
        ...
        for (int n = 0; n < res_w; n++)
        {
            ...
            // AG: Draw calls are merged together by the DrawList system. Still, we should render our batch are lower level to save a bit of CPU.
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    if (tab_list_popup_button)
        if (ImGuiTabItem* tab_to_select = TabBarTabListPopupButton(tab_bar)) // AG: Will alter BarRect.Min.x!
