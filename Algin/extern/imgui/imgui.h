...
struct ImVector
{
    ...
    // AG: It is illegal to call push_back/push_front/insert with a reference pointing inside the ImVector data itself! e.g. v.push_back(v[10]) is forbidden.
...
// AG: For basic min/max size constraint on each axis you don't need to use the callback! The SetNextWindowSizeConstraints() parameters are enough.
...
// AG: You most likely do NOT need to use draw callbacks just to create your own widget or customized UI rendering,
...
// (AG: the style and the naming convention here is a little inconsistent, we currently preserve them for backward compatibility purpose,
...
struct ImFontAtlas
{
    ...
    // AG: Make sure that your string are UTF-8 and NOT in your local code page.
    // Read https://github.com/ocornut/imgui/blob/master/docs/FONTS.md/#about-utf-8-encoding for details.
    // AG: Consider using ImFontGlyphRangesBuilder to build glyph ranges from textual data.
    ...
    // [Internal]
    // AG: Access texture data via GetTexData*() calls! Which will setup a default font for you.
