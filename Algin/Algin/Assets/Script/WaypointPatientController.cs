...
namespace Script
{
    ...
    public class WaypointPatientController : Script.Library.IScript
    {
        ...
        private const float MOVE_SPEED = 9.0f;
        ...
        private Vector3D _originalWp5;
        private Vector3D _dirWp4ToWp5;
        private float _bufferDistance = 5.0f;
        ...
        /// </summary>
        public void Initialize(int patientIndex, Vector3D wp1, Vector3D wp2, Vector3D wp3, Vector3D wp4, Vector3D wp5, Vector3D wp6, Vector3D wp7, Vector3D wp8, Vector3D wp9, Vector3D wp10)
        {
            ...
            // Offset wp5 backwards along the path from wp4 to wp5 so patients line up
            Vector3D dir = new Vector3D(wp4.x - wp5.x, wp4.y - wp5.y, wp4.z - wp5.z);
            ...
            {
                dir = new Vector3D(1, 0, 0); // fallback direction
            }
              // Reverse the patient index so patient 0 is at offset distance, and others follow
            _queueIndex = (patientIndex == 0) ? 0 : patientIndex; 
            
            _bufferDistance = 2.0f; // Reset buffer distance back to normal size
            _originalWp5 = wp5;
            _dirWp4ToWp5 = dir;
            _wp5 = wp5 + (dir * (_queueIndex * _bufferDistance));

            Logger.log(Logger.LogLevel.Info, 
                $"WaypointPatientController [{ObjectId}]: Debug - WP4({wp4.x},{wp4.y},{wp4.z}), original WP5({_originalWp5.x},{_originalWp5.y},{_originalWp5.z}), Dir({dir.x},{dir.y},{dir.z}), queueIndex={_queueIndex}");
            ...
            Logger.log(Logger.LogLevel.Info, 
                $"WaypointPatientController [{ObjectId}]: Init wp5={_wp5.x}, {_wp5.y}, {_wp5.z} with queue index {_queueIndex}");
        ...
        public void AdvanceQueue()
        {
            ...
            {
                _queueIndex--;
                _wp5 = _originalWp5 + (_dirWp4ToWp5 * (_queueIndex * _bufferDistance));
                Logger.log(Logger.LogLevel.Info, 
                    $"WaypointPatientController [{ObjectId}]: Queue advanced! New queue index: {_queueIndex}. New WP5: ({_wp5.x}, {_wp5.y}, {_wp5.z})");
                
                // If the patient is already heading to or at WP5, update their target to walk forward
                if (CurrentTarget == WaypointTarget.MovingToWaypoint5 || CurrentTarget == WaypointTarget.AtWaypoint5)
                {
                    ...
                    if (CurrentTarget == WaypointTarget.AtWaypoint5)
                    {
                        ...
                        if (smr != null) smr.PlayAnimation("walking_in_place");
                    }
                }
            }
        }
        ...
        public void MoveToCounter()
        {
            ...
            Script.Library.Object patient = new Script.Library.Object(ObjectId);
            SkinnedMeshRenderer smr = patient.getComponent<SkinnedMeshRenderer>();
            PatientManager.TryGetPatientData(ObjectId, out PatientData Pdata);

            if (CurrentTarget == WaypointTarget.AtWaypoint1)
            {
                _queueIndex = WaypointPatientControllerManager.GetNextQueueIndex();
                _wp5 = _originalWp5 + (_dirWp4ToWp5 * (_queueIndex * _bufferDistance));

                if(Pdata.meshNumber == 7)
                {
                    Logger.log(Logger.LogLevel.Warning,
                        $"VIP AUDIO PLAYING");
                    _ = DiagnosePanelMgr.Instance.PlayVipAudioSequenceAsync();
                }
        ...
        public void MoveToTesting()
        {
            ...
            if (CurrentTarget == WaypointTarget.AtWaypoint5)
            {
                ...
                Pdata.inPosition = true;
                PatientManager.SetPatientData(ObjectId, Pdata);

                smr.PlayAnimation("walking_in_place");

                //Logger.log(Logger.LogLevel.Info,
                //    $"WaypointPatientController [{ObjectId}]: Moving to WP6");
                Logger.log(Logger.LogLevel.Info, $"{Pdata.PatientName}: Moving to WP6");
        ...
        private void SfxIllness(string illnessName)
        {
            switch (illnessName)
            {
                case "No Illness":
                    Logger.log(Logger.LogLevel.Warning,$"{illnessName} 1");
                    break;

                case "Urinary tract infection":
                    Logger.log(Logger.LogLevel.Warning, $"{illnessName} 2");
                    break;

                case "Mild Flu":
                    Logger.log(Logger.LogLevel.Warning, $"{illnessName} 3");
                    break;

                case "Gastroenteritis":
                    Logger.log(Logger.LogLevel.Warning, $"{illnessName} 4");
                    break;

                case "Severe Allergy":
                    Logger.log(Logger.LogLevel.Warning, $"{illnessName} 5");
                    break;

                case "Blood Infection":
                    Logger.log(Logger.LogLevel.Warning, $"{illnessName} 6");
                    break;

                case "Drug-Induced Cardiac Stress":
                    Logger.log(Logger.LogLevel.Warning, $"{illnessName} 7");
                    break;

                case "Toxic-Induced Neurological Distress":
                    Logger.log(Logger.LogLevel.Warning, $"{illnessName} 8");
                    break;
        ...
        private void OnArriveAtWaypoint()
        {
            ...
            SkinnedMeshRenderer smr = patient.getComponent<SkinnedMeshRenderer>();
            PatientManager.TryGetPatientData(ObjectId, out PatientData Pdata);
            PatientManager.TryGetIllness(ObjectId, out Illness illness);
            ...
            switch (CurrentTarget)
            {
                ...
                case WaypointTarget.MovingToWaypoint5:
                    ...
                    DiagnosePanelMgr.Instance.PlayRandomDiagnoseSfx();
