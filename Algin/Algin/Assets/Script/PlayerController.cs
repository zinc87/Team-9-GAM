    ...
    public class PlayerController : Script.Library.IScript
    {
        ...
        public override void Update(double dt)
        {
            ...
            if (_isVipOverlayActive)
            {
                ...
                else
                {
                    ...
                    float t = timeAfterFadeIn / remainingDuration;
        ...
        private void UpdatePatientRay()
        {
            ...
            if (hitSomething && hit.layer == CollisionLayers.Layer_Computer)
            {
                ...
                if (ePressedOnce)
                {
                    if (PatientPanelMgr.Instance != null && PatientPanelMgr.Instance.IsOpen)
                    {
                    ...
                    {
                        PatientPanelMgr.Instance.OpenActivePatient();
                        GameState.SetCanOpenPatientUI(true);
            ...
            else if (hitSomething && hit.layer == CollisionLayers.Layer_NPC)
            {
                ...
                if (ePressedOnce)
                {
                    ...
                    Logger.log(Logger.LogLevel.Warning, $"In Position: {Pdata.inPosition}");

                    if (InventoryManager.Instance.CurrentItem == ItemType.Syringe && Pdata.inPosition)
                    {
                    ...
                    else if (InventoryManager.Instance.CurrentItem == ItemType.Cup && Pdata.inPosition)
                    {
                        Logger.log(Logger.LogLevel.Warning, $"{Pdata.PatientName}'s Urine Sample Collected");
                        TutorialTextMgr.Instance.triggar(TutorialTextMgr.promptState.Navigate_To_Lab_Rm);
                    ...
                    else if (InventoryManager.Instance.CurrentItem == ItemType.Test_Tube && Pdata.inPosition)
                    {
                ...
                if(Pdata.inPosition && (InventoryManager.Instance.CurrentItem != ItemType.Scanner))
                {
                    ApplyAlpha(pressEtoInteract, 1.0f);
                }
                if(InventoryManager.Instance.CurrentItem == ItemType.Scanner)
                    ApplyAlpha(pressEtoInteract, 0.0f);
            }
            ...
            // FIX: Removed PatientUIPanel reference and replaced it with PatientPanelMgr
            if (PatientPanelMgr.Instance != null && PatientPanelMgr.Instance.IsOpen)
            {
                ApplyAlpha(pressEtoInteract, 0.0f);
            }
