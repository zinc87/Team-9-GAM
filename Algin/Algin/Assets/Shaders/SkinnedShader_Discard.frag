...
uniform vec2 uScanCenterNDC;
uniform float uScanRadNDC;
uniform float uAspect;
uniform int uScanActive;
uniform int uScanAffect;
uniform int uScanActiveInt;
...
void main() {
    ...
    vec2 center = vec2(float(uScreenWidth) / 2.f, float(uScreenHeight) / 2.f); 

    float dist = distance(gl_FragCoord.xy, center);
    float radius = float(uScreenWidth) / 32.f;

    if (dist < radius) {
        discard;
    }

    if (uScanActiveInt == 1 && uScanAffect == 1)
    {   
        
        // Project fragment to clip space
        vec4 clipPos = uViewProj * vec4(fs.WorldPos, 1.0);
        vec2 ndcPos  = clipPos.xy / clipPos.w;   // [-1,1]
        
        // 2D distance in screen space
        vec2 delta = ndcPos - uScanCenterNDC;
        delta.x *= uAspect;
    ...
    if (uMaterial.hasSpecularTex == 1) {
        specularColor = texture(uSpecularTex, fs.TexCoord).rgb;
    }
    */
    ...
    for (int i = 0; i < count; ++i) {
        ...
        for (int s = 0; s < uNumShadowed; ++s) {
            ...
            if (uLightShadowIdx[s] == i) {
                ...
                vec3 shadowDir = normalize(-uLights[i].direction);
